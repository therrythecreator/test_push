...
/*   Updated: 2019/08/27 14:24:24 by yodana           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../includes/checker.h"

int		window_check(SDL_Event *event, char *mv, t_all *r, int m)
{
	int time;

	check_event(event, r);
	time = SDL_GetTicks();
	if (time - r->prev_time >= r->delay && m == 0)
	{
		r->prev_time = time;
